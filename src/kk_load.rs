...
impl PuzzleAsString {
    ...
    pub fn new_from_file(file_name: &str) -> Result<Self, String> {
        ...
        let raw_puzzle_string= match fs::read_to_string(file_name.trim()){
            ...
            Err(e) => return Err(format!("Error reading file. Error message:\n{}",e.to_string()))
        };
    ...
    pub fn get_dimension(&self) -> Result<usize, String> {
        ...
        let mut positions_list: Vec<usize> = self.puzzle_string
            .join(".")
        ...
        if position_count_dedup == positions_count && positions_count >=9 && positions_count <=81 {
            ...
            let dim:usize=positions_list.iter()
        ...
        Err(format!("Dimension can't be determined. Field is not consistent or completely specified.\n\
                Count of positions in file vs. count without duplicates: {},{}\
                Found positions{:?}", positions_count, position_count_dedup, positions_list))
    }
...
mod kk_loader_tests {
    ...
    fn check_load_from_file() {
        ...
        assert_eq!(valid_kenken_file.description, "Newdoku.com KenKen-puzzle nr.: 1278350 with Dim 4 x 4");
