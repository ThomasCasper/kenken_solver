...
impl Puzzle {
    ...
    fn initialize_kenken_from_definition(
        ...
        for cell_as_string in puzzle_string_vector {
            self.groups
                .push(Group::new_kenken(self.dimension, cell_as_string)?);
        }
    ...
    pub fn get_new_valid_field(&self) -> (Option<Self>, Option<Group>) {
        ...
        while index < new_cells.len() {
            ...
            let (opt_cnt, cell_pos, valid_cell) = new_cells
                .remove(index)
...
mod kk_group_tests {
    ...
    fn check_kenken_base() {
        ...
        assert_eq!(
            new_field.apply_option_to_field(&next_group_option.unwrap(), 0),
            true
        );
